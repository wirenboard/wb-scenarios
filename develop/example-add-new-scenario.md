...
- **[Процесс разработки сценария](development-process.md)** - этапы разработки, стиль кода и лучшие практики
...
Разработка нового сценария состоит из семи этапов (подробнее см. [development-process.md](development-process.md)):

0. **Планирование и подготовка** - сбор требований (в примере пропущен)
1. **Хардкод логики** — создаём модуль сценария с дефолтными значениями
2. **Создание JSON-схемы для UI** — добавляем описание в схему, проверяем WEBUI
...
## Этап 1. Хардкод логики сценария

> Этот этап необязателен при использовании AI-ассистента — можно сразу переходить к этапу 2.

На этом этапе создаём модуль сценария с бизнес-логикой, используя дефолтные
...
не возвращаться к этому потом.

### 1.1. Создание директории

```bash
mkdir scenarios/link-in-to-out
```

Все файлы общие для нескольких сценариев нужно располагать в папке `src`.

### 1.2. Написание скрипта

Пишем харкод скрипт логики сценария, создаем виртуальное устройство и реализуем логику через правила и функции.
Работу скрипта можно проверить, сохранив файл в `/mnt/data/etc/wb-rules/`, тем самым он автоматически подтянется,
запуститься и его можно будет увидеть в Устройствах и отслеживать его работу по логам.

---

## Этап 2. Создание JSON-схемы

### 2.1. Добавление описания

Открываем файл `schema/wb-scenarios.schema.json` и добавляем описание сценария
внутрь раздела `"definitions":` после последнего сценария.

Ниже в примере описание сценария с именем 'linkInToOut' - именно это имя будет
искать скрипт инициализации который мы напишем на следующем этапе.

Обратите внимание на:

- Имя сценария - написано в первой строке `"linkInToOut": {`
...
- Поле `scenarioType` - строка типа сценария, должна быть уникальной.
  ...
  то есть - так как у нас в первой строке написано `"linkInToOut": {` - то
...
### 2.2. Добавление в список сценариев
...
### 2.3. Добавляем переводы

Переводы делать обязательно.

Желательно делать это учитывая два момента:

- Создавать промежуточное название поля и уже это название использовать
...
- В конце файла `schema/wb-scenarios.schema.json` находим `"translations": {`
  ...
  строковых параметров не переводятся - поэтому там нужно написать текстом
  прямо в поле.
...
и при необходимости их изменение.

### 2.4. Проверка работы WEBUI

Проверка состоит из нескольких этапов:

- **Открывается ли конфигуратор** после изменении схемы

  Если синтаксис или структура файла были нарушены,
...
- **Отображается ли новый сценарий** в выпадающем списке

  ![Отображение нового сценария в выпадающем списке](example-new-scenario-in-dropdown.png)

- **Как отображается WEBUI** зайдя на страницу вашего сценария
...
- **Сохранить** - проверить что сохраняется и конфиг изменяется при ваших
  сохранениях в WEBUI
- **Проверить структуру конфиг файла** - все ли вас устраивает.
...
Если всё проверено и WEBUI корректно работает — можно переходить к следующему этапу.

---

### Этап 3. Создание модуля сценария
...
## Этап 4. Объединение схемы и логики

### 4.1. Создание модуля инициализации
...
### 4.2. Подключение к главному файлу
...
### 5.1. Выбор контролов для проверки
...
Сохраняем и начинаем проверять.
...
и добавлять тот функционал который нужен именно вам.
...
- Внутри функции `initializeScenario()` поправить создание экземпляра и обновить структуру `cfg` объекта так чтобы параметры отражали структуру вашей схемы
