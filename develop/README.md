...
2. Если в вашем локальном ридми сценария есть изображения, то для них нужно
   создать подпапку `<!scenario-name!>/img/`

3. Добавить описание схемы webui для вашего нового сценария
   в файл `schema/scenarios.schema.json`

4. Файлы изображений для отображения схеме в формате png - положить рядом
   в папке `schema/*.png`

5. Если требуется поменять конфиг `wb-scenarios.conf` чтобы он корректно
   открывался в соответствии с вашими изменениями в схеме.
   В данный момент конфиг поставляется пустым, поэтому этот пункт
   можно опустить.

## Описание процесса разработки сценария
...
### 1. Создать основную ветку `feature`
...
### 2. Создание веток для задач `sub-feature`
...
## Выбор порядка этапов разработки

После того как вы зальете первую версию сценария через PR содержащий папку
и README - дальше есть несколько возможных подходов - выберете удобный вам,
они в итоге приводят к одному результату, но разные начальные точки и разные
первые PR.

Три варианта от самого легкого новичкам к более трудным внизу.
...
## Порядок разработки разделенный по PR

### 1. Создание папки сценария и README
...
### 3. Добавляем новый сценарий в схему

Обратите внимание, что элементы схемы должны называться так же как переменные
в JS - в нашем случае используется `camelCase`.

#### 3.1. В definitions добавляем описание
...
function init(deviceTitle, cfg) {
  /** TODO: Add validation logic + scenario logic*/
...
#### 6.1. Вспомогательные фукнции

Для однотипности, в модуле сценариев используется несколько стандартных
вспомогательных методов, внутри которых описываются валидация входящих
параметров и генерация имен используемых в сценарии.

Примеры:

- Валидация параметров
  Принимает на вход объект конфигурации и возвращает `false`
  в случае невадной конфигурации:
...
пользователь заполнил корректно конфигурацию топиков
и выбранных событий и действий
Для этого нам нужно проверить:

- Существование введённых пользователем устройства и топиков
  ...
  В этот момент топики в MQTT могут отсутствовать. Это нормально, особенно
  во время старта контроллера. В некоторых случаях можно попробовать решить
  введя умную задержку в создании правила выполнив проверку наличия
  (примерно через 1-2 секунды)

  Выдавать ошибку и прекращать работу тут стоит только, если топики
  уже есть и не совпадают с нужными например по типу. Так как простое
  отсутствие топиков может быть не только их реальным отстутствием в системе,
  а просто не верным порядкоминициализации виртуальных девайсов и контролов.
...
Функция `isConfigValid()` должна показать пользователю **все ошибки сразу**,
а не по одной за раз - поэтому не используйте тут return внутри проверок,
а возвращайте только одно значение в конце функции.
...
### 7. Заполнить `README.md` сценария
...
### 8. Добавить ссылку на сценарий в общий `README.md`
