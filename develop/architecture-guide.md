...
Виртуальное устройство сценария называется `wbsc_<idPrefix>`. Префикс `wbsc_`
зарезервирован за `wb-scenarios`: сценарии из `/etc/wb-scenarios.conf` занимают
свои имена при загрузке системных правил, то есть раньше файлов из
`/etc/wb-rules/`. Скрипт, объявивший устройство с этим префиксом, получит от
wb-rules ошибку `Device with given ID already exists` и не загрузится.

### Имя не проверяется перед созданием

Это не упущение, менять не нужно.

wb-rules запускается без флага `-cleanup` и при остановке не удаляет
retained-топики своих виртуальных устройств. Раньше это компенсировалось тем, что
`scenario-init-main.js` удалял топики всех известных устройств перед созданием
сценариев. Компенсация была ненадёжной по двум причинам:

- список удаляемых имён собирался только из успешно созданных VD, поэтому одна
  неудача выбрасывала имя из списка навсегда — и остатки под этим именем больше
  никто не чистил, а проверка не давала сценарию подняться уже никогда;
- список в принципе не покрывает случаи, когда имя занял кто-то другой: например
  MQTT-мост, возвращающий копии топиков с удалённого брокера, или сторонний
  скрипт.

Сейчас топики живых устройств намеренно не удаляются — это оставило бы устройство
неполным (см. ниже). Поэтому на старте имя штатно занято собственными остатками
с прошлой сессии, и `getDevice()` их находит. Предварительная проверка имени
превратила бы рядовой рестарт в неустранимый отказ, поэтому в `createBasicVd()`:

```javascript
// НЕПРАВИЛЬНО - падает на собственных вчерашних топиках
if (getDevice(vdName) !== undefined) {
  return null;
}
var vdObj = defineVirtualDevice(vdName, vdCfg);

// ПРАВИЛЬНО - бесхозные топики wb-rules забирает сам, а имя,
// занятое живым устройством, отдаёт исключением
try {
  vdObj = defineVirtualDevice(vdName, vdCfg);
} catch (err) {
  /* настоящий конфликт: у имени есть живой владелец */
}
```

### Уборка брошенных топиков

Топики устройства, которое больше никем не создаётся (например, сценарий удалили
из конфига), забрать некому — их удаляет `scenario-init-main.js` **после**
инициализации всех сценариев и только для тех записей реестра, которые создал он
сам. Живые устройства не трогаются: стирание их топиков оставит устройство
неполным, потому что контролы, которые никто не перезаписывает позднее, заново не
публикуются.

Служебные ключи реестра, на которых это построено, описаны в
[хранилище сценариев](scenario-persistent-storage.md).
