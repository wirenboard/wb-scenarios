...
Это не упущение, менять не нужно. wb-rules запускается без флага `-cleanup` и
при остановке не удаляет retained-топики своих виртуальных устройств. Поэтому на
следующем старте устройство `wbsc_<idPrefix>` уже присутствует в системе — как
внешнее, собранное драйвером из оставшихся топиков. То же даёт MQTT-мост,
возвращающий копии топиков с удалённого брокера.

`getDevice()` такие устройства находит, и предварительная проверка имени
превращает штатную ситуацию в неустранимый отказ: сценарий не поднимается, пока
топики не вычистят вручную. Поэтому в `createBasicVd()`:
