...
function whenCrossUpper(topic, cfg, ctx) {
  ...
  var isEventTriggered =
    topic.val.new > cfg.actionValue && topic.val.prev <= cfg.actionValue;
...
Object.keys(registryEventResolvers).forEach(function (key) {
  ...
  if (!resolver.resetResolverName) {
    log.warning(
      'resetResolverName для действия "' + key + '" не установлен'
    );
    return;
  }
  ...
  resolver.resetResolver =
    registryEventResolvers[resolver.resetResolverName].launchResolver;
});
