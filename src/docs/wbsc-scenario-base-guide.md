...
2.  Инициализация и запуск сценария\
    Эта часть должна быть вынесена в отдельный файл скрипта wb-rules
...
Поле `State` рабочего сценария ведет базовый класс, руками его в
`initSpecific()` выставлять не нужно. Базовый класс создает правило на
`rule_enabled` и пишет в `State` результат `computeState(isEnabled)` - при
выключенном тумблере "Отключен", при включенном "Активен".

`computeState(isEnabled)`:
Второй опциональный метод. Переопределяется, если у сценария больше двух
рабочих состояний. Позиция тумблера приходит аргументом, читать контрол
внутри метода не нужно. Примеры переопределения - периодический таймер
отдает "Ожидает" вне активного окна, термостат и ПИД-регулятор отдают
"Используемый канал в ошибке", пока ошибка канала не снята.
...
CustomScenario.prototype.initSpecific = function () {
  ...
  ruleId = defineRule({
    ...
    then: function (newValue) {
      ...
      log.info(
        'Зафиксировано изменение контрола ' +
          self.cfg.controlTopic +
          ' (всего изменений: ' +
          currentCount +
          ')'
      );
