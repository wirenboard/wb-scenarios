...
Сразу после инициализации целевая температура берется из конфигуратора,
но ее можно изменить с помощью ползунка в виртуальном устройстве.

Обогреватель работает по гистерезису.
...
- При деактивации нагреватель будет выключен
- При при активации будет обновлено актуальное показание температуры, и далее
  нагреватель будет включен только если температура датчика ниже выставленной
