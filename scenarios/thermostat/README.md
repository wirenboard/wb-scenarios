...
выключения внутри фиксированного периода ШИМ.
...
- pwmPeriodSec — период ШИМ в секундах (целое число).
- pidRecalcCycles — как часто пересчитывать PID (каждые N циклов ШИМ).
...
- Длительности цикла (output_timing) — строка вида "on / off", показывающая расчётное время включения и выключения в текущем периоде ШИМ (в секундах). Только для чтения.
  Пример: wbsc_bathroom__heat_floor/output_timing
- Сброс ПИД (pid_reset) — кнопка для обнуления интегральной суммы ПИД-регулятора и перезапуска цикла ШИМ.
