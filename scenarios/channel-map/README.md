...
Дополнительно у каждой связки есть флаг `inverted` (по умолчанию `false`).
...
При `inverted: true` копирование выполняется так же, как при
`inverted: false`, но дополнительно применяется логическое НЕ
...
- **Конфликт смешанной инверсии** — на одной паре каналов нельзя
  одновременно иметь связки с `inverted: true` и `inverted: false`:
...
| `inverted`   | boolean | Необязательное (по умолчанию `false`). Инвертировать значение при копировании. Цель направления должна быть `switch`/`alarm`; для `both` оба канала. |
...
1. **Без трансформаций значения:** поддерживается копирование и
   логическая инверсия (`inverted: true`, цель должна быть
...
2. `mqttTopicsLinks` {array} — массив связок. Минимум 1 элемент. Каждый элемент:
   ...
   - `inverted` {boolean}: необязательное (по умолчанию `false`).
