...
Дополнительно у каждой связки есть флаг `invertSourceValue` (по умолчанию `false`).
Если он установлен, при копировании значение логически инвертируется
(см. ниже). Старые конфигурации без этого поля работают как раньше —
...
### Инверсия значения (`invertSourceValue`)

При `invertSourceValue: true` копирование выполняется так же, как при
`invertSourceValue: false`, но дополнительно применяется логическое НЕ
поверх стандартной булевой конверсии. Например:

- `true` → `false`,
- число `0` → `true` (`Boolean(0) === false`, затем `!`),
- строка `"hello"` → `false` (`Boolean('hello') === true`, затем `!`),
- строка `"0"`/`"false"` → `true` (спец-кейс bool-конверсии, затем `!`).

Источник может быть любого типа. Требования предъявляются только
к **целевому** каналу выбранного направления — он должен быть
`switch` или `alarm`:

- `forward` (A → B): канал B должен быть `switch`/`alarm`, A любой;
- `backward` (B → A): канал A должен быть `switch`/`alarm`, B любой;
- `both` (A ↔ B): оба канала должны быть `switch`/`alarm` —
  направление идёт в обе стороны, инверсия применяется в обе.

`pushbutton` как цель не поддерживается (контрол stateless, инверсия
не имеет смысла). Числовые и строковые цели тоже запрещены — для них
инверсия неоднозначна. При нарушении этих требований валидация
...
- **Конфликт смешанной инверсии** — на одной паре каналов нельзя
  одновременно иметь связки с `invertSourceValue: true` и `invertSourceValue: false`:
  это либо запись противоположных значений в один target, либо
  бесконечный каскад. Валидация в этом случае возвращает ошибку
  и сценарий не инициализируется.
...
| `invertSourceValue` | boolean | Необязательное (по умолчанию `false`). Инвертировать значение при копировании. Цель направления должна быть `switch`/`alarm`; для `both` оба канала. |
...
| `state`        | value  | Состояние: «Активен» / «Отключен»    |
...
1. **Без трансформаций значения:** поддерживается копирование и
   логическая инверсия (`invertSourceValue: true`, цель должна быть
   `switch`/`alarm`). Масштабирование, смещение и пользовательские
   формулы пока не поддерживаются.
...
2. `mqttTopicsLinks` {array} — массив связок. Минимум 1 элемент. Каждый элемент:
   ...
   - `invertSourceValue` {boolean}: необязательное (по умолчанию `false`).
     Инвертировать значение при копировании. Цель направления
     должна быть `switch`/`alarm`; для `both` — оба канала.
