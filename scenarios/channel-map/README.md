...
- **Дубликаты и перекрытия** — если одна и та же пара каналов
  ...
  - при перевёрнутом порядке каналов (A/B ↔ B/A) направление
...
используется механизм счётчика поколений (generation counter):
каждое пользовательское нажатие — новое «поколение». Все записи
в рамках одной волны помечаются одним номером поколения. Если
контрол уже помечен текущим поколением — повторная запись
пропускается.
