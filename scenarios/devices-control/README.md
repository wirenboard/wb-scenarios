...
- @param {Array<string>} reqCtrlTypes - Required Control Types
  ...
  события/действия
- события имеют @param {function} handler - предикат события,
  а действия имеют @param {function} launchHandler - выполнить действие
  и @param {function} resetHandler - откатить действие
